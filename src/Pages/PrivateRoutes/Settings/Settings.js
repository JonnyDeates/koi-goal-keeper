...
function Settings({bgColor}) {
    const themes = ["Default", "Bekko", "Benigoi","Kigoi","Kin Showa","Lucki","Platinum","Sanke"]

    return <div style={bgColor} className={'content'}>
        <SettingsConsumer>{({state, dispatch}) => (
            <>
                <h1 className={'title tab'} style={{...bgColor}}>Profile {state.nickname}</h1>
                <h2 className={'setting-label theme-selector'}>Email:
                    <span>{state.email}</span>
                </h2>
                <Nickname nickname={state.nickname} dispatch={dispatch}/>
                <Password dispatch={dispatch} />
                <h2 className={'setting-label theme-selector'}>Theme
                    <Dropdown currentSet={state.theme} items={themes} color={getAccentColor(state)} darkmode={state.darkmode}
                              setCurrent={(s)=>dispatch({type: "patchData", value: s, key: 'theme', callback: ()=>{}})} isColor={true}/>
                </h2>
                <AdvancedSettings dispatch={dispatch} state={state}/>
            </>)}
        </SettingsConsumer>
    </div>
}
